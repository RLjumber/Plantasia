...
const Plants = () => {
    ...
    const fetchPlants = async (page) => {
        await fetch(`https://perenual.com/api/species-list?page=${page}&key=${process.env.REACT_APP_API_KEY}`)
        .then(response => {
        ...
        <div className="all_plants_container">
            {plants.length > 0 && (
                <div key={plants.id} className="plant_container">
                    {plants.map(plant => (
                        <Link key={plant.id} to={`/plants/${plant.id}`} className="plant_div">
            ...
            <form className="page_form" onSubmit={onSubmit}>
                <label htmlFor="go_to">Go to Page: </label>
